...
def _convert(currency):
    try:
        availables = _reimport(update=False)
    except IOError:
        availables = _reimport(update=False)
...
class Pymarketcap(object):
    ...
    def ticker(self, currency=None, convert=None,
        ...
        response = session.get(url, timeout=1).json()
        if currency:
            response = response[0]
    ...
    def markets(self, currency, VERBOSE=False, V=False):
        ...
        for m in marks:
            volume_24h = int(m.find('span', {'class': 'volume'}).getText().replace('$', '').replace(',', '').replace('*', ''))
            price = float(m.find('span', {'class': 'price'}).getText().replace('$', '').replace(',', '').replace('*', ''))
            childs = m.contents
